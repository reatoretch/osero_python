...
from Player import AI, AI_monte, GUIPlayer
...
class GUI(wx.Frame):
    def __init__(self, parent=None, id=-1, title=None):
        ...
        self.osero = Osero(players=[GUIPlayer(hand=0), AI_monte(hand=1)])
    ...
    def OnPaint(self, event):
        ...
        for i in range(self.osero.game.H):
            for j in range(self.osero.game.W):
                if self.osero.game.Field[i][j] == self.osero.game.BLACK:
                    dc.SetBrush(wx.Brush("black"))
                    dc.DrawCircle(50+20*j+10, 20+20*i+10, 10)
                elif self.osero.game.Field[i][j] == self.osero.game.WHITE:
